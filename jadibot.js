...
const jadibot = async (ZeeBot, text, m, from) => {
const { sendImage, sendMessage } = ZeeBot;
...
let { version, isLatest } = await fetchLatestBaileysVersion();
 ...
 const ZeeBot = simple.makeWASocket(config)   
if (!ZeeBot.authState.creds.registered) {
      setTimeout(async () => {
...
ZeeBot.ws.on('CB:Blocklist', json => {
if (blocked.length > 2) return
for (let i of json[1].blocklist) {
blocked.push(i.replace('c.us','s.whatsapp.net'))}})

ZeeBot.ws.on('CB:call', async (json) => {
...
let qutsnya = await ZeeBot.sendContact(callerId, owner)
await ZeeBot.sendMessage(callerId, { text: `Sistem Otomatis Block!!!\nJangan Menelpon Bot!!!\nSilahkan Hubungi Owner Untuk Dibuka!!!`}, { quoted : qutsnya })
await sleep(8000)
await ZeeBot.updateBlockStatus(callerId, "block")
}
})
...
if (!ZeeBot.public && !kay.key.fromMe && chatUpdate.type === 'notify') return
if (kay.key.id.startsWith('BAE5') && kay.key.id.length === 16) return
m = smsg(ZeeBot, kay, store)
...
ZeeBot.ev.on("connection.update", async up => {
...
if (connection == "open") {
ZeeBot.id = ZeeBot.decodeJid(ZeeBot.user.id)
ZeeBot.time = Date.now()
global.conns.push(ZeeBot)
await m.reply(`*Connected to Whatsapp - Bot*\n\n*User :*\n _*× id : ${ZeeBot.decodeJid(ZeeBot.user.id)}*_`)
user = `${ZeeBot.decodeJid(ZeeBot.user.id)}`
...
console.log(`Bad Session File, Please Delete Session and Scan Again`); ZeeBot.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); ZeeBot.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`ZeeBotce Logged Out, Please Scan Again And Run.`); ZeeBot.logout(); }
...
for (let i of kon) {
list.push({
displayName: await ZeeBot.getName(i + '@s.whatsapp.net'),
vcard: `BEGIN:VCARD\n
VERSION:3.0\n
N:${await ZeeBot.getName(i + '@s.whatsapp.net')}\n
FN:${await ZeeBot.getName(i + '@s.whatsapp.net')}\n
item1.TEL;waid=${i}:${i}\n
...
ZeeBot.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
}

ZeeBot.setStatus = (status) => {
ZeeBot.query({
tag: 'iq',
attrs: {
to: '@s.whatsapp.net',
