...
parser.add_argument ("-apk", type=str, help= "Nama Apk", required=True )
parser.add_argument ("-pilih", type=str, help= "Pilih Apk" )
...
def apkpure(app_name=None):
	try:
		app_name = app_name if app_name else apknya
		while not app_name:
			app_name = apknya
		...
		if search_result_data:
			...
			daemon_dl = threading.Thread(target=downloader,args=(f'https://d.apkpure.net/b/APK/{package}?version=latest',f"{name}_{package}_ByZEEBOT"))
			daemon_dl.start()
			if daemon_dl.is_alive():
				print(f"============================")
			print(f"Done, Aplikasi berhasil didownload\nBot Sedang mengirim file. Mohon ditunggu!")
